...
from cantherm.core import q_tr, q_rot, q_vib
...
@pytest.mark.parametrize("freqs, temp, q_vib_ans", [(ethane_freqs, 298.15, 1.060e00)])
def test_Q_vib(freqs, temp, q_vib_ans):
    q_vib_test = q_vib(freqs, temp)
    npt.assert_approx_equal(q_vib_test, q_vib_ans, significant=4)
