...
from cantherm.core import s_tr, s_rot, s_vib
...
@pytest.mark.parametrize("freqs, temp, s_vib_ans", [(ethane_freqs, 298.15, 0.6462)])
def test_S_vib(freqs, temp, s_vib_ans):
    s_vib_test = s_vib(freqs, temp)
    npt.assert_approx_equal(s_vib_test, s_vib_ans, significant=4)
