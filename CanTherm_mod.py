...
def main():
  ...
  for line in lines:
    tokens = line.split()
    piecesep.append(double(tokens[1]))
    piecesep.append(double(tokens[2]))
    piecesep.append(double(tokens[3]))
  inpu.close()
  ...
  for i in range(len(data.MoleculeList)):
    ...
    if molecule.numRotors != 0:
      ...
      for j in range(len(Temp)):
        if i == 0:
          Entropy[i * len(Temp) + j] = Entropy[i *
                                               len(Temp) + j] + piecesep[j * 3]
          Cp[i * len(Temp) + j] = Cp[i * len(Temp) + j] + piecesep[j * 3 + 2]
          Thermal[i * len(Temp) + j] = Thermal[i *
                                               len(Temp) + j] + piecesep[j * 3 + 1]
        else:
          Entropy[i * len(Temp) + j] = Entropy[i * len(Temp) + j] + ent[j]
          Cp[i * len(Temp) + j] = Cp[i * len(Temp) + j] + cp[j]
          Thermal[i * len(Temp) + j] = Thermal[i * len(Temp) + j] + dh[j]
...
#         Partition[i*len(Temp)+j] = Thermal[i*len(Temp)+j]+q[j]
    ...
    for j in range(len(Temp)):
      Entropy[i * len(Temp) + j] = Entropy[i * len(Temp) + j] + \
          1.985 * math.log(molecule.nelec)
  ...
  for j in range(len(Temp)):
    if (data.ReacType == 'Unimol'):
      rate[j] = (1.381e-23 * Temp[j] / 6.626e-34) * math.exp((Entropy[len(Temp) + j] - Entropy[j]) / 1.985) * math.exp(-(data.MoleculeList[1].Energy +
                                                                                                                         Thermal[len(Temp) + j] / 627.5095 - data.MoleculeList[0].Energy - Thermal[j] / 627.5095) * 627.5095 * 1.0e3 / 1.985 / Temp[j])
      # wigner correction
      rate[j] *= (1.0 + 1.0 / 24.0 *
